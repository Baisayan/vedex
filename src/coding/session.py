...
from ai.events import ProviderErrorEvent, ProviderResponseEndEvent, ProviderTextDeltaEvent
from coding.branch_summary import summarize_branch_messages_with_model
...
from coding.credentials import FileCredentialStore, credentials_path
...
from coding.provider_config import (
    ProviderConfig,
    ProviderConfigError,
    ProviderSettings,
...
from coding.provider_runtime import ClosableModelProvider, create_model_provider
...
from coding.session_export import (
...
class SessionResources:
    """Vedex-owned resources loaded around a coding session."""
...
class CodingSessionConfig:
    ...
    resource_paths: VedexResourcePaths | None = None
...
class CodingSession:
    """Vedex's coding-agent environment wrapper.
    ...
    def available_providers(self) -> tuple[str, ...]:
        """Return provider names Vedex can call with available credentials."""
    ...
    def available_model_choices(self) -> tuple[ModelChoice, ...]:
        """Return provider/model choices Vedex can call with available credentials."""
        if self._provider_settings is None:
    ...
    def context_window_tokens(self) -> int:
        """Return the active model's configured context window, or Vedex's fallback."""
        provider = self._active_provider_config()
        if provider is None:
...
def _resolve_export_destination(
    ...
    *,
    ...
    if destination is None:
        ...
        return cwd / f"Vedex-session.{format}"

    resolved = destination if destination.is_absolute() else cwd / destination
    if resolved.suffix:
        return resolved
    name = session_path.stem if session_path is not None else "Vedex-session"
    return default_session_export_artifact_path(
        Path(name),
...
def _session_export_title(session: CodingSession) -> str:
    ...
    if manager is not None and session_id is not None:
        ...
        if record is not None and record.title:
            return record.title
    return f"Vedex session {session_id}" if session_id is not None else "Vedex Session Export"
...
def _load_session_resources(
    resource_paths: VedexResourcePaths,
...
def default_session_path(cwd: Path) -> Path:
    """Return Vedex's default user-home session path for a project cwd."""
