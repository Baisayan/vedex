...
from ai.env import DEFAULT_OPENAI_COMPATIBLE_BASE_URL

from coding.credentials import FileCredentialStore, credentials_path
from coding.paths import VedexPaths
from coding.provider_catalog import BUILTIN_PROVIDER_CATALOG
...
type ProviderConfig = OpenAICompatibleProviderConfig
...
def provider_settings_path(paths: VedexPaths | None = None) -> Path:
    return (paths or VedexPaths()).home / "providers.json"


def load_provider_settings(paths: VedexPaths | None = None) -> ProviderSettings:
...
def save_provider_settings(settings: ProviderSettings, paths: VedexPaths | None = None) -> Path:
    path = provider_settings_path(paths)
...
def provider_kind(provider: ProviderConfig) -> Literal["openai-compatible"]:
...
def _validate_thinking_config(
    *,
    ...
    if thinking_parameter not in {
        ...
        raise ProviderConfigError(
            "Provider thinking_parameter must be reasoning_effort or reasoning.effort."
        )
...
def _optional_thinking_parameter(
    ...
    raise ProviderConfigError(
        f"Provider field must be reasoning_effort or reasoning.effort: {field_name}"
    )
