from __future__ import annotations

from pathlib import Path

import pytest
from vedex.environments import LocalEnvironment
from vedex.schema import JSONValue
from vedex.tools import ToolInputError, create_read_tool

from .conftest import run_async


def test_read_text_offsets_limits_and_continuation_hints(
    tmp_path: Path,
    local_environment: LocalEnvironment,
) -> None:
    path = tmp_path / "notes.txt"
    path.write_text("one\ntwo\nthree\nfour", encoding="utf-8")
    tool = create_read_tool(environment=local_environment)

    first = run_async(tool.execute({"path": "notes.txt", "limit": 2}))
    second = run_async(tool.execute({"path": "notes.txt", "offset": 3, "limit": 1}))

    assert first.content.startswith("     1  one\n     2  two")
    assert "Use offset=3" in first.content
    assert second.content.startswith("     3  three")
    assert "Use offset=4" in second.content


def test_read_rejects_invalid_paths_offsets_and_limits(
    tmp_path: Path,
    local_environment: LocalEnvironment,
) -> None:
    (tmp_path / "directory").mkdir()
    (tmp_path / "file.txt").write_text("line", encoding="utf-8")
    tool = create_read_tool(environment=local_environment)

    invalid_arguments: list[dict[str, JSONValue]] = [
        {"path": "missing.txt"},
        {"path": "directory"},
        {"path": "file.txt", "offset": 0},
        {"path": "file.txt", "limit": 0},
        {"path": "file.txt", "limit": 2_001},
        {"path": "file.txt", "offset": 2},
        {"path": 3},
    ]
    for arguments in invalid_arguments:
        with pytest.raises(ToolInputError):
            run_async(tool.execute(arguments))


def test_read_truncates_many_lines_and_large_single_line(
    tmp_path: Path,
    local_environment: LocalEnvironment,
) -> None:
    many_lines = tmp_path / "many.txt"
    many_lines.write_text("\n".join(str(index) for index in range(2_001)), encoding="utf-8")
    huge_line = tmp_path / "huge.txt"
    huge_line.write_text("x" * (51 * 1024), encoding="utf-8")
    tool = create_read_tool(environment=local_environment)

    line_result = run_async(tool.execute({"path": "many.txt"}))
    huge_result = run_async(tool.execute({"path": "huge.txt"}))

    assert "1 more lines in file. Use offset=2001" in line_result.content
    assert "exceeds 50.0KB limit" in huge_result.content
    assert "Use bash to inspect it" in huge_result.content


@pytest.mark.parametrize("content", [b"\x89PNG\r\n\x1a\n", b"text\0binary"])
def test_read_rejects_non_utf8_or_binary_content(
    tmp_path: Path,
    content: bytes,
    local_environment: LocalEnvironment,
) -> None:
    path = tmp_path / "binary.dat"
    path.write_bytes(content)
    tool = create_read_tool(environment=local_environment)

    with pytest.raises(ToolInputError, match="not valid UTF-8 text"):
        run_async(tool.execute({"path": "binary.dat"}))


def test_read_rejects_absolute_and_escaping_paths(
    tmp_path: Path,
    local_environment: LocalEnvironment,
) -> None:
    path = tmp_path / "notes.txt"
    path.write_text("ordinary text", encoding="utf-8")
    tool = create_read_tool(environment=local_environment)

    for invalid_path in (str(path), "../notes.txt"):
        with pytest.raises(ToolInputError, match="workspace"):
            run_async(tool.execute({"path": invalid_path}))
