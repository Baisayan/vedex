...
async def test_prompt_appends_user_message_and_assistant_response() -> None:
    ...
    harness = AgentHarness(
        AgentHarnessConfig(provider=provider, model="fake", system="You are Vedex.")
    )
...
def test_messages_property_returns_immutable_snapshot() -> None:
    harness = AgentHarness(
        AgentHarnessConfig(provider=FakeProvider([]), model="fake", system="You are Vedex."),
...
async def test_harness_passes_tools_to_loop() -> None:
    ...
    harness = AgentHarness(
        AgentHarnessConfig(provider=provider, model="fake", system="You are Vedex.", tools=[tool])
    )
